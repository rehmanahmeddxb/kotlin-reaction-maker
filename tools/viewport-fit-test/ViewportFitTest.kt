...
object ViewportFitTest {
    ...
    fun main(args: Array<String>) {
        ...
        // ---- 7. merge regression: Step 5 panel + Phase 2 floating controls ----
        // Test the production budget, not a duplicated formula in the Activity.
        // Values are dp at density 1; scaling all dimensions preserves the rule.
        for (h in listOf(640, 800, 1024)) {
            for (top in listOf(64, 96)) {
                for (fixedSheet in listOf(174, 234)) {
                    for (controls in listOf(0, 68)) {
                        val tag = "merged chrome h=$h top=$top sheet=$fixedSheet controls=$controls"
                        val panel = ViewportFit.panelHeight(h, top, fixedSheet, controls)
                        val bottom = fixedSheet + controls + panel
                        check("$tag: panel <= 38% cap", panel >= 0 && panel <= h * 0.38f)
                        check("$tag: canvas keeps >= 28% of screen", h - top - bottom >= h * 0.28f)
                        check("$tag: extra controls cannot grow the panel",
                            ViewportFit.panelHeight(h, top, fixedSheet, controls + 48) <= panel)
                        for ((label, sw, sh) in aspects) {
                            val fit = ViewportFit.contain(360f, h.toFloat(), 0, top, 0, bottom,
                                6f, sw, sh, 96f)
                            check("$tag $label: no panel/toolbar overlap or fallback",
                                fit.left >= 5.99f && fit.right <= 354.01f &&
                                    fit.top >= top + 5.99f && fit.bottom <= h - bottom - 5.99f,
                                "$fit")
                            check("$tag $label: aspect preserved",
                                near(fit.width / fit.height, sw.toFloat() / sh, 0.002f))
                        }
                    }
                }
            }
        }
        check("panel budget: zero-height view", ViewportFit.panelHeight(0, 64, 200, 68) == 0)
        check("panel budget: negative-height view", ViewportFit.panelHeight(-1, 64, 200, 68) == 0)
        check("panel budget: chrome fills the view", ViewportFit.panelHeight(360, 100, 240, 68) == 0)
        check("panel budget: generous viewport uses 38% cap", ViewportFit.panelHeight(2000, 64, 174, 68) == 760)
        check("panel budget: invalid negative chrome clamps",
            ViewportFit.panelHeight(1000, -10, -10, -10) == ViewportFit.panelHeight(1000, 0, 0, 0))
